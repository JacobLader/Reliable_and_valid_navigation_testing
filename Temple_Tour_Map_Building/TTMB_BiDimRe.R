...
#here we do this using either A key or B key (one at a time) but it is possible to do both
key <- read.csv(file = "A_key.csv", header = TRUE)
...
DependentVariables <- read.csv(file = "A2_Processed_TTMB_6-6-24.csv", header = TRUE)
...
write.csv(BiDimReR_Results, file = "A2_BiDimReR_Results.csv", row.names=FALSE)
