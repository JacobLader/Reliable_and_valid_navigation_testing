...
# Step 1: read in data ####
filelist <- list.files(path="input", pattern="*.csv", full.names=T)
data <- lapply(filelist, function(x)read.table(x, header=TRUE, sep=","))

# Step 2: organize data & extract XY coordinates ####
# save just xy coordinates and image ID
data_xy <- lapply(data, function(x)x[c("participant","date",
...
# this throws a warning but it works correctly:
...
# save data to csv
write.csv(data_long_sorted, "drag_drop_processed_data.csv")
